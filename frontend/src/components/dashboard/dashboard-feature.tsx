...
const LearnMoreModal = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => (
  <Modal isOpen={isOpen} onClose={onClose}>
    <h2 className="text-2xl font-bold text-purple-400 mb-4">About Our Blockchain Car Marketplace</h2>
    <div className="text-gray-300 space-y-4">
      <p>Welcome to the future of automotive transactions! Our platform leverages blockchain technology to create a transparent, secure, and efficient car buying experience.</p>
      
      <h3 className="text-xl font-semibold text-purple-400 mt-6">Key Features:</h3>
      <ul className="list-disc pl-5 space-y-2">
        <li>Verified Vehicle History: Every car's history is immutably recorded on the blockchain</li>
        <li>Smart Contracts: Automated, secure transactions with no intermediaries</li>
        <li>Decentralized Authentication: Transparent verification of vehicle ownership</li>
        <li>Real-time Updates: Instant access to vehicle information and market data</li>
      </ul>

      <h3 className="text-xl font-semibold text-purple-400 mt-6">How It Works:</h3>
      ...
      <p className="mt-6">Join us in revolutionizing the automotive industry with blockchain technology!</p>
    </div>
  </Modal>
);

const CarDetailsModal = ({ isOpen, onClose, car }: { isOpen: boolean; onClose: () => void; car: Listing }) => (
  <Modal isOpen={isOpen} onClose={onClose}>
    <div className="space-y-4">
      <img src={car.image} alt={car.name} className="w-full h-64 object-cover rounded-xl" />
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <h3 className="text-lg font-semibold text-purple-400 mb-2">Vehicle Details</h3>
            <ul className="space-y-2 text-gray-300">
              <li><span className="text-gray-400">Make:</span> {car.make}</li>
              <li><span className="text-gray-400">Model:</span> {car.model}</li>
              <li><span className="text-gray-400">Year:</span> {car.year}</li>
              <li><span className="text-gray-400">Condition:</span> {car.condition}</li>
              <li><span className="text-gray-400">Mileage:</span> {car.mileage}</li>
              <li><span className="text-gray-400">VIN:</span> {car.vin}</li>
            </ul>
          </div>

          <div>
            <h3 className="text-lg font-semibold text-purple-400 mb-2">Specifications</h3>
            <ul className="space-y-2 text-gray-300">
              <li><span className="text-gray-400">Type:</span> {car.type}</li>
              <li><span className="text-gray-400">Transmission:</span> {car.transmission}</li>
              <li><span className="text-gray-400">Fuel Type:</span> {car.fuelType}</li>
              <li><span className="text-gray-400">Exterior:</span> {car.exteriorColor}</li>
              <li><span className="text-gray-400">Interior:</span> {car.interiorColor}</li>
            </ul>
          </div>
        </div>
...
const categories = [
  'All', 'Electric', 'SUV', 'Sedan', 'Pickup Truck', 'Luxury', 
  'Crossover', 'Hybrid', 'Diesel', 'Coupe', 'Hatchback', 'Wagon', 'Convertible'
];

export default function DashboardFeature() {
  const { listings: allListings, totalListings, loading } = useMockBlockchainData();
  ...
  useEffect(() => {
    ...
    if (zipCode.trim()) {
      filtered = filtered.filter(car => car.zipCode.startsWith(zipCode.substring(0, 3)));
    }
    ...
    switch (sortOrder) {
      ...
      case 'newest':
      default:
        filtered.sort((a, b) => parseInt(b.id.slice(2)) - parseInt(a.id.slice(2)));
    }

    setFilteredListings(filtered);
  }, [allListings, selectedCategory, condition, make, model, zipCode, sortOrder]);
  ...
  return (
    <div className="min-h-screen bg-black text-white">
     ...
     {/* Search Section */}
      ...
      {/* Categories */}
      <div className="max-w-7xl mx-auto px-4 py-8">
        <h2 className="text-2xl font-bold mb-6 text-purple-400">Popular categories</h2>
          ...
          {categories.map((category, index) => (
            <button
              ...
              className={`px-6 py-2 rounded-full transition-all duration-300 transform hover:scale-105 
                ...
                ${category === selectedCategory ? 'bg-purple-600 text-white' : 'bg-gray-800 text-purple-300 hover:bg-gray-700'}`}
            >
              {category}
            </button>
          ))}
      ...
      <div id="results" className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h2 className="text-2xl font-bold text-purple-400">Featured Listings</h2>
            ...
            {filteredListings.map((car) => (
              ...
              >
                ...
                <div className="flex justify-between items-center mt-2">
                  ...
                  <span className="text-lg font-bold text-purple-300">{car.price}</span>
                </div>
                <button 
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="text-center">
              <div className="text-4xl font-bold text-purple-400 mb-2">{totalListings}</div>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            <div>
              ...
              <p className="text-gray-400">Revolutionizing the automotive industry through blockchain technology and transparent transactions.</p>
            </div>
            <div>
              ...
              <ul className="space-y-2">
                ...
                <li><a href="#" className="text-gray-400 hover:text-purple-400 transition-colors">Featured Cars</a></li>
                <li><a href="#" className="text-gray-400 hover:text-purple-400 transition-colors">Sell Your Car</a></li>
            ...
            <div>
              <h3 className="text-xl font-bold text-purple-400 mb-4">Newsletter</h3>
